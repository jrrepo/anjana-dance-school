...
export default function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-[#fff1d6]/15 bg-[#070b21]/82 shadow-[0_18px_55px_rgba(0,0,0,0.32)] backdrop-blur-xl">
      <nav className="mx-auto flex min-h-20 w-[min(1180px,calc(100%-32px))] items-center justify-between gap-5" aria-label="Main navigation">
        <a className="flex h-16 shrink-0 items-center" href="/" aria-label="Anjana Dance School home">
          <img
            className="h-full w-auto drop-shadow-[0_0_18px_rgba(246,182,74,0.25)]"
            src={logo.src}
            width={logo.width}
            height={logo.height}
            alt="Anjana Dance School logo"
          />
        </a>
      ...
      >
        <div className="min-h-0">
          <div className="mx-auto grid w-[min(100%-32px,420px)] gap-2 py-5">
            <img className="mb-3 w-36 rounded-lg shadow-[0_0_28px_rgba(246,182,74,0.16)]" src={logo.src} width={logo.width} height={logo.height} alt="Anjana Dance School logo" />
            {links.map((link) => (
              <a
                key={link.href}
